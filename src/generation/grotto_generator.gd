...
static func create(seed_value: int, base: int, fixed_final: int = -1, version: int = VERSION) -> Dictionary:
	...
	return {"generator_version": version, "seed": seed_value, "base_quality": base,
		...
		"id": "LA%d-%08X-%03d" % [version, seed_value, quality]}
...
static func generate(metadata: Dictionary) -> Array:
	assert(SUPPORTED_VERSIONS.has(int(metadata.generator_version)), "Unsupported generator version")
	var result: Array = []
	for i in range(int(metadata.depth) + 1):
		result.append(FloorGenerator.generate(metadata, i) if int(metadata.generator_version) == 1 else FloorGeneratorV2.generate(metadata, i))
	return result
